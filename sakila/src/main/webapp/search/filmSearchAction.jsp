...
<body>
	...
	<!-- 숫자페이지 만들기 -->
		...
		// currentPage가 1보다 크면 이전이 보이도록 하고, 이전을 누르면 currentPage에서 -1값을 보냄
			if(currentPage > 1) {
		%>
				<li><a href="<%=request.getContextPath()%>/film/filmSearchAction.jsp?currentPage=<%=currentPage-1%>&categoryName=<%=categoryName%>&rating=<%=rating%>&price=<%=price%>&length=<%=length%>&title=<%=title%>&actor=<%=actor%>" class="btn btn-outline-info">이전</a></li>
		<%
			}
		// currentPage가 lastPage보다 작으면 다음이 보이도록 하고, 다음을 누르면 currentPage에서 +1값을 보냄
			if(currentPage < lastPage) {
		%>
				<li><a href="<%=request.getContextPath()%>/film/filmSearchAction.jsp?currentPage=<%=currentPage+1%>&categoryName=<%=categoryName%>&rating=<%=rating%>&price=<%=price%>&length=<%=length%>&title=<%=title%>&actor=<%=actor%>" class="btn btn-outline-info">다음</a></li>
		<%
			}
		%>		</ul>
</body>
